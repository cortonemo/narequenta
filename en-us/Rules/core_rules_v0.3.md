...
## 3. Acting in the World (Action Resolution)
When you do something meaningful, you choose the **Motor ($\mathbf{E_{P}}$)** and the **Quality ($\mathbf{E_{S}}$)**.

Action Resolution uses **E_cur**:
- Roll $\mathbf{d100 \le \mathbf{E_{cur}}}$ of the Motor ($\mathbf{E_{P}}$) to succeed.

### Action Cost (Attrition)
1. You **spend** $\mathbf{E_{cur}}$ by the Attrition Cost ($\mathbf{A_{rate}}$) of the **E_P** plus the fixed cost ($1\%$-$2\%$) of the $\mathbf{E_{S}}$.
2. **Success costs the most.** The only question is "what will it cost me next time?"
...
## 4. Scarring / Refinement (Reallocation)
**Refinement is a strategic choice.** It occurs at Decay Milestones ($\mathbf{E_{max}}$ at $84\%$, $69\%$, etc.).

Refining does two things:
- Permanently **LOWERS** the **Maximum ($\mathbf{E_{max}}$)** of that Essence (via $10\%$ Reallocation).
- **IMPROVES** the **Efficiency ($\mathbf{A_{rate}}$)** of that Essence (reduces the Attrition cost based on the **Waning Scale**).

This is how characters "improve." They don't get bigger. They get **sharper and smaller.**
